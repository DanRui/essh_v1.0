...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#log_keepTime_form').form({
            url: '${ctx}/sys/log!updateKeepTime.action',
        ...
        log_datagrid = $('#log_datagrid').datagrid({
            url: '${ctx}/sys/log!datagrid.action',
            fit:true,
...
//                $(".easyui-layout").layout('expand','north');
    ...
    function showKeepTimeDialog(){
        var dialogUrl = "${ctx}/sys/log!time.action";
        //弹出对话窗口
        ...
        if (rows.length > 0) {
            $.messager.confirm('确认提示！', '您确定要删除当前选中的所有行？', function (r) {
                if (r) {
                    ...
                    $.post('${ctx}/sys/log!remove.action', {ids: ids},
                            function (data) {
                                if (data.code == 1) {
     ...
     */
    function delAll(){
        $.post('${ctx}/sys/log!removeAll.action',
            function (data) {
                if (data.code == 1) {
                    log_datagrid.datagrid('clearSelections');//取消所有的已选择项
                    log_datagrid.datagrid('load');//重新加载列表数据
...
<div class="easyui-layout" fit="true" style="margin: 0px;border: 0px;overflow: hidden;width:100%;height:100%;">
    ...
    <div data-options="region:'north',title:'过滤条件',collapsed:true,split:false,border:false"
        ...
        <form id="log_search_form" style="padding: 0px;">
            ...
            登录名：<input type="text" name="filter_LIKES_loginName" placeholder="请输入登录名..."
