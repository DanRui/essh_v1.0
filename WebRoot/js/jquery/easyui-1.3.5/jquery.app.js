...
(function ($) {
	...
	function initApp(target, apps) {
		...
		for (var i = 0;i<apps.length;i++) {
			...
			var icon = $('<img/>').height(opts.iconSize).width(opts.iconSize).attr('class', app.iconCls).appendTo(appItem);
	...
	function refresh(target, href) {
		...
		if (href) {
			$.ajax({
				...
				error : function (XMLHttpRequest, textStatus, errorThrown) {
					$.messager.alert("", textStatus || errorThrown, "数据格式错误.");
				}
			});
		}
	}
	...
	function initMenu(target, menus) {
        ...
        //叠加其它开始菜单项
        if(opts.otherStartMenus){
            for(var i=0;i<opts.otherStartMenus.length;i++) {
                if (opts.otherStartMenus[i] == '-') {
                    var sep = $('<div class="menu-sep"></div>');
                    startMenuDiv.append(sep);
                } else{
                    startMenuDiv.menu('appendItem',opts.otherStartMenus[i]);
                }
            }
        }
	...
	$.fn.app.methods = {
		...
		closeapp : function (appId) {
			$('div[w_id="' + appId + '"]').window("close");
		},
		openapp : function (appId) {
			$('div[w_id="' + appId + '"]').window("open");
		},
        refreshapp : function (appId) {
            var winIframe = $('div[w_id="' + appId + '"] iframe');
	...
	$.fn.app.defaults = {
        ...
        otherStartMenus:null,// startMenu URL之外其它开始菜单配置项 menu options
		...
		iframeOpen : false,
		loadUrl : { //远程数据加载路径
			app : ctx+'/js/jquery/easyui-1.3.5/apps.json', //app数据
			startMenu : ctx+'/js/jquery/easyui-1.3.5/startMenu.json', //开始菜单数据
			widget : ctx+'/js/jquery/easyui-1.3.5/widget.json'
		},
		lang : { //国际化
			initLayout : "初始化布局",
